...
//						ATOI(digits[digit_count - 1 - i]);
	...
	if (second_used) {
		...
		int i;
		for (i = 0; i < IPV6_LENGTH_NO_SCOPE - (len_first + len_second); i++)
			*end_first_part++ = 0;
...
buffer::Buffer IP::toBuffer() const {
	if (this->isValid()) {
		if (IP_V4 == this->version) {
			return buffer::Buffer((const char*) addrBuffer, 4);
		} else {
			if (this->hasV6Scope()) {
				return buffer::Buffer((const char*) addrBuffer, IPV6_LENGTH_WITH_SCOPE);
			} else {
				return buffer::Buffer((const char*) addrBuffer, IPV6_LENGTH_NO_SCOPE);
			}
		}
	}
