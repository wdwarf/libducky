...
static int Baudrate(int baudrate) {
	...
	THROW_EXCEPTION(SerialException,
			"Unsuppotr baudrate value: " + Variant(baudrate).toString(),
			baudrate);
}
...
int Serial::send(char* buf, int len) {
	fd_set fs;
	FD_ZERO(&fs);
	FD_SET(this->fd, &fs);
	timeval tv;
...
EParity Serial::getParity() const {
...
void Serial::setParity(EParity parity) {
	/** 设置校验位 **/
	switch (parity) {
	case PARITY_NONE:
	...
	case PARITY_EVEN:
		...
		break;
	default:
		THROW_EXCEPTION(SerialException, "Invalid parity.", parity);
	}
...
EStopBit Serial::getStopBit() const {
...
void Serial::setStopBit(EStopBit stopBit) {
	/** 设置停止位 **/
	switch (stopBit) {
	case STOPBIT_1:
	case STOPBIT_1P5:
	...
	case STOPBIT_2:
		...
		break;
	default:
		THROW_EXCEPTION(SerialException, "Invalid stop bit.", stopBit);
	}
