    ...
    while( p && *p ) {
        ...
        if ( decl ) {
                ...
                if ( !_document->NoChildren() ) {
                	...
                	/**  性能文件中会出现xml-stylesheet，tinyxml不支持，需要改动一下, add by zhangweibin 2015-10-29  **/
                	const char *pValue = node->Value();
            		std::string value("");
            		if (pValue) value = pValue;
                	if (std::string::npos == value.find("xml-stylesheet"))
                	{
...
XMLError XMLDocument::SaveFile( const char* filename, bool compact )
{
    ...
    /**  add by zhangweibin 2015-11-11 **/
	if (XML_SUCCESS == _errorID)
	{
		fflush(fp);
	}
